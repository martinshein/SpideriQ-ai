...
| A folder-bound menu item reads back with `source` set and `children` empty | **Nothing — this is by design.** The authenticated read returns the *binding*; only the public site read expands it | Edit `source`. Do **not** write the expanded pages back as `children` — that replaces the rule with a snapshot and the menu permanently stops tracking your pages |
| A page you just created isn't in a folder-bound menu | A bound menu lists **published** pages only — a draft is structurally excluded, not filtered out late | Publish the page. There is nothing to add to the menu |
| A folder-bound node renders as plain text with no link | That folder has no published children, so there is no landing page to point at | Publish a page inside the folder. Emitting no `url` is deliberate — it avoids a dead `href=""` |
| `content_update_navigation` 422s and *none* of your edits save | A menu is written as a full replacement, so one `kind:"folder"` item missing `folder_id` fails the whole payload | Set `folder_id` on every folder-bound item before writing. Fetch → modify → write back the complete list |
| `content_create_page` with `is_folder: true` then `content_publish_page` → 404 | Folders are dashboard-only organisers with no URL; publishing one is refused deliberately | Don't publish folders. Publish the pages *inside* them — that is what makes them appear in a bound menu |
| `{% for child in folder.children %}` renders nothing on a page you expected it to | `folder` is only present when that page **is** its folder's resolved landing page; on any other page the key is absent, and iterating an unknown variable is an empty loop with no error | Guard with `{% if folder %}`, and check the page really is the folder's landing page (first published child by position, or the pinned `index_page_id`) |
